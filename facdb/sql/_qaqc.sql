...
SELECT a.*,
	b.wkb_geometry INTO prev_full
from dcp_facilities_with_unmapped a
	LEFT JOIN dcp_facilities b on a.uid = b.uid;
...
select coalesce(a.opabbrev, b.opabbrev) as opabbrev,
	coalesce(a.opname, b.opname) as opname,
	coalesce(a.optype, b.optype) as optype,
	coalesce(a.datasource, b.datasource) as datasource,
	b.count_old,
	a.count_new - b.count_old as diff INTO qc_operator
from new a
	join old b on a.opabbrev = b.opabbrev
...
select coalesce(a.overabbrev, b.overabbrev) as overabbrev,
	coalesce(a.overagency, b.overagency) as overagency,
	coalesce(a.overlevel, b.overlevel) as optype,
	coalesce(a.datasource, b.datasource) as datasource,
	b.count_old,
	a.count_new - b.count_old as diff INTO qc_oversight
from new a
	join old b on a.overabbrev = b.overabbrev
...
select coalesce(a.facdomain, b.facdomain) as facdomain,
	...
	b.count_old,
	a.count_new - b.count_old as diff INTO qc_classification
from new a
	join old b on a.facdomain = b.facdomain
...
with new as (
	SELECT captype,
		sum(capacity::integer) as sum_new
	FROM facdb
	group by captype
),
old as (
	SELECT captype,
		sum(capacity::integer) as sum_old
	FROM prev_full
	group by captype
)
select a.captype,
	a.sum_new,
	b.sum_old,
	a.sum_new - b.sum_old as diff INTO qc_captype
from new a
	join old b on a.captype = b.captype;
-- Report changes in number of mapped records
...
select coalesce(a.facdomain, b.facdomain) as facdomain,
	...
	coalesce(b.wogeom_old, 0) as wogeom_old INTO qc_mapped
from geom_new a
	FULL join geom_old b on a.facdomain = b.facdomain
	AND a.facgroup = b.facgroup
	AND a.facsubgrp = b.facsubgrp
...
select coalesce(a.facdomain, b.facdomain) as facdomain,
	...
	coalesce(count_new, 0) - coalesce(count_old, 0) as diff INTO qc_diff
FROM (
		select facdomain,
			facgroup,
			facsubgrp,
...
SELECT a.datasource,
	...
	a.raw_record_counts - b.final_record_counts as diff INTO qc_recordcounts
FROM (
		SELECT source as datasource,
...
SELECT facsubgrp,
	...
	) - count(distinct bin) as count_repeat_bin,
	count(*) filter (
		where bin is null
	) as count_null_bin,
	count(*) filter (
		where bin::text like '%000000'
	) as count_million_bin,
	count(*) filter (
		where geom is null
	) as count_wo_geom INTO qc_subgrpbins
FROM facdb
GROUP BY facsubgrp
