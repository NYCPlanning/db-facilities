...
WITH merged AS (
    SELECT nysed_activeinstitutions.*,
    ...
    FROM nysed_activeinstitutions
        LEFT JOIN nysed_nonpublicenrollment ON trim(
...
SELECT uid,
    ...
    (
        CASE
            ...
            WHEN inst_type_description = 'CUNY' THEN CONCAT(
                'CUNY - ',
                initcap(right(inst_sub_type_description, -5))
            )
            WHEN inst_type_description = 'SUNY' THEN CONCAT(
                'SUNY - ',
                initcap(right(inst_sub_type_description, -5))
            )
    ...
    (
        CASE
            ...
            WHEN inst_sub_type_description LIKE '%PRE-SCHOOL%'
            AND (
            ...
            WHEN (inst_type_description LIKE 'PUBLIC%')
            OR (inst_sub_type_description LIKE 'PUBLIC%') THEN 'Public K-12 Schools'
            WHEN (inst_type_description LIKE '%COLLEGE%')
